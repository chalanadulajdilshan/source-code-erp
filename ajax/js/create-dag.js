jQuery(document).ready(function () {
  ...
  $("#casingCost, #quantity").on("input", function () {
    calculateDagItemTotal();
  });
  ...
  function calculateDagItemTotal() {
    const cost = parseFloat(document.getElementById("casingCost").value) || 0;
  ...
  function addDagItem() {
    ...
    const beltDesignId = document.getElementById("beltDesign").value;
    const beltDesignText = document.getElementById("beltDesign").selectedOptions[0]?.text || '';
    const casingCost = parseFloat(document.getElementById("casingCost").value) || 0;
    ...
    const noRow = document.getElementById("noDagItemRow");
    if (noRow) noRow.remove();

    const row = document.createElement("tr");

    row.innerHTML = `
        <td>${vehicleNo}<input type="hidden" name="vehicle_no[]" value="${vehicleNo}"></td>
        <td>${beltDesignText}<input type="hidden" name="belt_design_id[]" value="${beltDesignId}"></td>
        <td>${casingCost.toFixed(2)}<input type="hidden" name="casing_cost[]" value="${casingCost}"></td>
        <td>${barcode}<input type="hidden" name="barcode[]" value="${barcode}"></td>
    ...
    document.getElementById("dagItemsBody").appendChild(row);
    resetDagInputs();
    updateSubTotal();
  }
  function resetDagInputs() {
    document.getElementById("vehicleNo").value = '';
    document.getElementById("beltDesign").value = '';
    document.getElementById("casingCost").value = '';
  ...
  // Recalculate grand total whenever subtotal or discount changes
  ...
  // Delegated click event for dynamically added "Remove" buttons
  $(document).on("click", ".remove-item", function () {
    const $row = $(this).closest("tr");
    $row.remove();

    // If no rows left, show "No items added"
    ...
    updateSubTotal(); // Recalculate totals
  });
