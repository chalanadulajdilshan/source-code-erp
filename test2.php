 ...
 if ($_POST["Command"] == "proces") {
     ...
     $year = substr($_POST["DTPicker1"], 0, 4);
     $month = substr($_POST["DTPicker1"], 5, 2);

     $dd = date("m/Y", strtotime($_POST["DTPicker1"]));
     $sql_rs = "select * from vatrate where  sdate <='" . $_POST["DTPicker1"] . "' order by sdate desc";
     ...
     // and REF_NO !='BEN/CR/170838/21'
 
     if (($_POST['monthwise'] == "nch") && ($_POST['monthwise1'] == "ch")) {
         exit('please select correct date rage or check box error');
     }

     if (($_POST['monthwise'] == "nch") && ($_POST['monthwise1'] == "nch")) {
         $sql_rssslma = "select * from s_salma where (Brand='CHENG SHING' OR Brand='ETERNOPRESA') and Accname != 'NON STOCK'   and  month(sdate1)='" . intval(date("m", strtotime($_POST["DTPicker1"]))) . "' and  year(sdate1)='" . date("Y", strtotime($_POST["DTPicker1"])) . "' and C_CODE = '" . trim($_POST["txt_cuscode"]) . "' and   CANCELL='0'";


     }
     if (($_POST['monthwise'] == "ch") && ($_POST['monthwise1'] == "nch")) {
         $sql_rssslma = "select * from s_salma where (Brand='CHENG SHING' OR Brand='ETERNOPRESA') and Accname != 'NON STOCK'   and  ((month(sdate1)='" . intval(date("m", strtotime($_POST["DTPicker1"]))) . "' and  year(sdate1)='" . date("Y", strtotime($_POST["DTPicker1"])) . "') or (month(sdate1)='" . intval(date("m", strtotime($_POST["DTPicker2"]))) . "' and  year(sdate1)='" . date("Y", strtotime($_POST["DTPicker2"])) . "') ) and C_CODE = '" . trim($_POST["txt_cuscode"]) . "' and   CANCELL='0'";
     }

     if (($_POST['monthwise'] == "ch") && ($_POST['monthwise1'] == "ch")) {
         $sql_rssslma = "select * from s_salma where (Brand='CHENG SHING' OR Brand='ETERNOPRESA') and Accname != 'NON STOCK'   and  ((month(sdate1)='" . intval(date("m", strtotime($_POST["DTPicker1"]))) . "' and  year(sdate1)='" . date("Y", strtotime($_POST["DTPicker1"])) . "') or (month(sdate1)='" . intval(date("m", strtotime($_POST["DTPicker2"]))) . "' and  year(sdate1)='" . date("Y", strtotime($_POST["DTPicker2"])) . "') or (month(sdate1)='" . intval(date("m", strtotime($_POST["DTPicker3"]))) . "' and  year(sdate1)='" . date("Y", strtotime($_POST["DTPicker3"])) . "') ) and C_CODE = '" . trim($_POST["txt_cuscode"]) . "' and   CANCELL='0'";
     }



     // if ($_POST['monthwise'] == "nch") {
     //     $sql_rssslma = "select * from s_salma where (Brand='CHENG SHING' OR Brand='ETERNOPRESA') and Accname != 'NON STOCK'   and  month(sdate1)='" . intval(date("m", strtotime($_POST["DTPicker1"]))) . "' and  year(sdate1)='" . date("Y", strtotime($_POST["DTPicker1"])) . "' and C_CODE = '" . trim($_POST["txt_cuscode"]) . "' and   CANCELL='0'";
     // } else {
     //     $sql_rssslma = "select * from s_salma where (Brand='CHENG SHING' OR Brand='ETERNOPRESA') and Accname != 'NON STOCK'   and  ((month(sdate1)='" . intval(date("m", strtotime($_POST["DTPicker1"]))) . "' and  year(sdate1)='" . date("Y", strtotime($_POST["DTPicker1"])) . "') or (month(sdate1)='" . intval(date("m", strtotime($_POST["DTPicker2"]))) . "' and  year(sdate1)='" . date("Y", strtotime($_POST["DTPicker2"])) . "') ) and C_CODE = '" . trim($_POST["txt_cuscode"]) . "' and   CANCELL='0'";
     // }
     // echo $_POST['monthwise'].'/';
     // echo $sql_rssslma.'/';
     $TXTTOT = 0;
     $totincen = 0;
     $totout = 0;
     ...
     while ($row_rssslma = mysql_fetch_array($result_rssslma)) {
         ...
         if ($_POST["cmbtype"] == "MAXXIS MC TYRE") {
                         ...
                         if ($row_rsbrand_mas["delinrate"] >= 0) {
                             ...
                             while ($row_rssttr = mysql_fetch_array($result_rssttr)) {
                                 ...
                                 if ($TypeGrid1[$ii][9] <= $TypeGrid1[$ii][10]) {
                                     $TypeGrid1[$ii][12] = ($row_rssttr["ST_PAID"] / 100 * $_POST["txt_percentage"]) / ($txt_vat + 100) * 100;
                                 } else {
                                     $TypeGrid1[$ii][12] = 0;
                                 }
                         ...
                         if ($row_rsbrand_mas["delinrate"] >= 0) {
                             ...
                             while ($row_rssttr = mysql_fetch_array($result_rssttr)) {
                                 ...
                                 if ((is_null($row_rssttr["st_chdate"]) == false) and ($row_rssttr["st_chdate"] != "0000-00-00")) {
                                     ...
                                     //echo strtotime($row_rssttr["st_chdate"])."/".strtotime($mdate);
                                     $diff = abs(strtotime($row_rssttr["st_chdate"]) - strtotime($mdate));
                                     $days = dateDifference($row_rssttr["st_chdate"], $mdate, $differenceFormat = '%a');
                                     $days = floor($diff / (60 * 60 * 24));
                                     $TypeGrid1[$ii][9] = $days;
                                     ...
                                     $TypeGrid1[$ii][7] = $row_rssttr["ST_DATE"];
                                     //echo strtotime($row_rssttr["ST_DATE"])."/".strtotime($mdate);
                                     $diff = abs(strtotime($row_rssttr["ST_DATE"]) - strtotime($mdate));
                                     $days = dateDifference($row_rssttr["ST_DATE"], $mdate, $differenceFormat = '%a');
                                     $days = floor($diff / (60 * 60 * 24));
                                     $TypeGrid1[$ii][9] = $days;
                                 ...
                                 //                                $sqldate = "select * from br_trn where brand ='" . $row_rsbrand_mas["class"] . "' and cus_code = '" . $row_rsVENDOR["CODE"] . "' and Rep='" . $row_rsVENDOR["rep"] . "' order by days desc";
////echo $sqldate;
//                                
//                                $result_days = mysql_query($sqldate, $dbinv);
//                                if ($row_days = mysql_fetch_array($result_days)) {
//                                    $TypeGrid1[$ii][10] = $row_days['days'];
//                                }
             ...
             if ((intval(date("m", strtotime($_POST["DTPicker1"]))) >= 1) and (date("Y", strtotime($_POST["DTPicker1"])) > 2010)) {
     ...
     if (($_POST['monthwise'] == "nch") && ($_POST['monthwise1'] == "nch")) {
         $sql_RScbal = "select * from c_bal where (brand='CHENG SHING' OR brand='ETERNOPRESA') and month(sdate1)='" . intval(date("m", strtotime($_POST["DTPicker1"]))) . "' and  year(sdate1)='" . date("Y", strtotime($_POST["DTPicker1"])) . "' and CANCELL='0'and CUSCODE='" . trim($_POST["txt_cuscode"]) . "' AND trn_type != 'ARN' and trn_type != 'REC' and trn_type != 'DGRN' and flag1 != '1' order by sdate1";
     }
     if (($_POST['monthwise'] == "ch") && ($_POST['monthwise1'] == "nch")) {
         $sql_RScbal = "select * from c_bal where (brand='CHENG SHING' OR brand='ETERNOPRESA') and ((month(sdate1)='" . intval(date("m", strtotime($_POST["DTPicker1"]))) . "' and  year(sdate1)='" . date("Y", strtotime($_POST["DTPicker1"])) . "') or (month(sdate1)='" . intval(date("m", strtotime($_POST["DTPicker2"]))) . "' and  year(sdate1)='" . date("Y", strtotime($_POST["DTPicker2"])) . "') ) and CANCELL='0'and CUSCODE='" . trim($_POST["txt_cuscode"]) . "' AND trn_type != 'ARN' and trn_type != 'REC' and trn_type != 'DGRN' and flag1 != '1' order by sdate1";
     }

     if (($_POST['monthwise'] == "ch") && ($_POST['monthwise1'] == "ch")) {
         $sql_RScbal = "select * from c_bal where (brand='CHENG SHING' OR brand='ETERNOPRESA') and ((month(sdate1)='" . intval(date("m", strtotime($_POST["DTPicker1"]))) . "' and  year(sdate1)='" . date("Y", strtotime($_POST["DTPicker1"])) . "') or (month(sdate1)='" . intval(date("m", strtotime($_POST["DTPicker2"]))) . "' and  year(sdate1)='" . date("Y", strtotime($_POST["DTPicker2"])) . "') or (month(sdate1)='" . intval(date("m", strtotime($_POST["DTPicker3"]))) . "' and  year(sdate1)='" . date("Y", strtotime($_POST["DTPicker3"])) . "') ) and CANCELL='0'and CUSCODE='" . trim($_POST["txt_cuscode"]) . "' AND trn_type != 'ARN' and trn_type != 'REC' and trn_type != 'DGRN' and flag1 != '1' order by sdate1";
     }
                 ...
                 if ($row_rsbrand_mas = mysql_fetch_array($result_rsbrand_mas)) {
                     if ($row_rsbrand_mas["delinrate"] == 2.5) {
                         if ($row_rsbrand_mas["delinrate"] >= 0) {
                             ...
                             $TypeGrid1[$ii][3] = $row_RScbal["AMOUNT"];
                             $TypeGrid1[$ii][5] = $row_RScbal["brand"];
                             $TypeGrid1[$ii][12] = -1 * ($TypeGrid1[$ii][3] / 100 * $_POST["txt_percentage"]) / ($row_RScbal["vatrate"] + 100) * 100;
                             $TypeGrid1[$ii][13] = -1 * ($TypeGrid1[$ii][3] / 100 * $_POST["txt_percentage"]) / ($row_RScbal["vatrate"] + 100) * 100;
                             $TypeGrid1[$ii][14] = $row_RScbal["trn_type"];
     ...
     $xx = trim(intval(date("m", strtotime($_POST["DTPicker1"]))));

     if (($_POST['monthwise'] == "nch") && ($_POST['monthwise1'] == "nch")) {
         $sql_rsincen = "select * from ins_payment where  I_month ='" . intval(date("m", strtotime($_POST["DTPicker1"]))) . "'  and I_year='" . date("Y", strtotime($_POST["DTPicker1"])) . "' and cusCode = '" . trim($_POST["txt_cuscode"]) . "'    and Type = '" . trim($_POST["cmbtype"]) . "' order by id desc ";
     }
     if (($_POST['monthwise'] == "ch") && ($_POST['monthwise1'] == "nch")) {
         $sql_rsincen = "select * from ins_payment where  ((I_year='" . date("Y", strtotime($_POST["DTPicker1"])) . "' and I_month='" . intval(date("m", strtotime($_POST["DTPicker1"]))) . "') or (I_year='" . date("Y", strtotime($_POST["DTPicker2"])) . "' and I_month='" . intval(date("m", strtotime($_POST["DTPicker2"]))) . "')) and cusCode = '" . trim($_POST["txt_cuscode"]) . "'    and Type = '" . trim($_POST["cmbtype"]) . "' order by id desc ";
     }

     if (($_POST['monthwise'] == "ch") && ($_POST['monthwise1'] == "ch")) {
         $sql_rsincen = "select * from ins_payment where  ((I_year='" . date("Y", strtotime($_POST["DTPicker1"])) . "' and I_month='" . intval(date("m", strtotime($_POST["DTPicker1"]))) . "') or (I_year='" . date("Y", strtotime($_POST["DTPicker2"])) . "' and I_month='" . intval(date("m", strtotime($_POST["DTPicker2"]))) . "') or (I_year='" . date("Y", strtotime($_POST["DTPicker3"])) . "' and I_month='" . intval(date("m", strtotime($_POST["DTPicker3"]))) . "')) and cusCode = '" . trim($_POST["txt_cuscode"]) . "'    and Type = '" . trim($_POST["cmbtype"]) . "' order by id desc ";
     }
     ...
     if ($row_rsincen = mysql_fetch_array($result_rsincen)) {
         ...
         if (is_null($row_rsincen["type"]) == false) {
             if (trim($row_rsincen["type"]) == trim($_POST["cmbtype"])) {
     ...
     if (date("Y", strtotime($_POST["DTPicker1"])) > 2012) {
         ...
         $sql_rsper = "Select * from intper where sdate <= '" . $_POST["DTPicker1"] . "'  and traget < " . (($txttot1 - $txt_grn) / ($txt_vat + 100) * 100) . " order by traget desc ";
         ...
         if ((intval(date("m", strtotime($_POST["DTPicker1"]))) >= 1) and (date("Y", strtotime($_POST["DTPicker1"])) >= 2017)) {

             $sql_rsper = "Select * from intper where incen_year = 2017 and traget < '" . ($txttot1 - $txt_grn) / ($txt_vat + 100) * 100 . "' order by traget desc ";
         } else {
...
//echo $TypeGrid1[10][13];
     ...
     while ($i < $TypeGrid1_count) {
         ...
         if ($row_TTR = mysql_fetch_array($result_TTR)) {
             ...
             $color = "orange";
             $sql_sch = "Select * from s_cheq where CR_CHNO = '" . $row_TTR['ST_CHNO'] . "'   and CR_C_CODE = '" . $_POST['txt_cuscode'] . "'   and CR_CHEVAL = '" . str_replace(',', '', $TypeGrid1[$i][8]) . "' and CR_BANK = '" . $row_TTR['st_chbank'] . "' and CR_FLAG='0'      ";

             $result_sch = mysql_query($sql_sch, $dbinv);
             if ($row_sch = mysql_fetch_array($result_sch)) {
                 $sql_salma = "select * from s_salma where REF_NO = '" . $row_TTR["ST_INVONO"] . "'";
                 $result_salma = mysql_query($sql_salma, $dbinv);
                 ...
                 $result_chsttr = mysql_query($sql_chsstr, $dbinv);
                 ...
                 $sql_invch = "Select * from s_invcheq where cus_code = '" . $_POST['txt_cuscode'] . "'  and cheque_no = '" . $row_TTR['ST_CHNO'] . "' and refno='" . $row_TTR['ST_REFNO'] . "' and   che_date='" . $row_TTR['st_chdate'] . "' and bank = '" . $row_TTR['st_chbank'] . "' order by che_date desc    ";
     ...
     $sql = "select * from dealer_incen_rmk where d_code = '" . $_POST["txt_cuscode"] . "'";
